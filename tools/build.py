...
SITE_URL is intentionally empty because the site is published on GitHub Pages without a custom domain.
"""
...
def render(page, slug, body, standalone=False):
    ...
    canonical = SITE_URL + ("/" if slug == "index.html" else "/" + slug) if SITE_URL else ""
    site_metadata = ""
    if SITE_URL:
        site_metadata = f"""<link rel="canonical" href="{canonical}">
