...
This site is configured for GitHub Pages without a custom domain. It uses relative
links and does not hard-code a domain, so it works whether the repository is a user
site (`username.github.io`) or a project site (`username.github.io/repository`).

If you later buy a custom domain, set `SITE_URL` in both `tools/build.py` and
`tools/assets.py`, then run:
